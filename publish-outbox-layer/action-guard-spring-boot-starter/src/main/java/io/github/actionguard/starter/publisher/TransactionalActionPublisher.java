 ...
 *
 * <p>
 * 它位于“core 可靠落库能力 -> Spring 事务与消息投递”的衔接边界上：内部委托
 * {@code DefaultActionPublisher} 先创建 action / step / outbox 记录，再根据当前事务状态决定
 * 何时把 outbox 真正投递到执行通道。
 *
 * <p>
 * 这个类存在的核心原因是避免脏消息。只要当前线程处于事务中，它就把 MQ 发送动作延后到
...
public class TransactionalActionPublisher implements ActionPublisher {
    ...
    public TransactionalActionPublisher(
        ...
        this.actionOutboxRepository = Objects.requireNonNull(actionOutboxRepository,
                "actionOutboxRepository must not be null");
        this.actionExecutionMessageProducer = Objects.requireNonNull(actionExecutionMessageProducer,
                "actionExecutionMessageProducer must not be null");
        this.publishRetryMaxAttempts = Math.max(1, publishRetryMaxAttempts);
        this.outboxDispatcher = new ActionOutboxDispatcher(actionOutboxRepository, actionExecutionMessageProducer,
                actionObservabilityService, Clock.systemUTC());
    }
